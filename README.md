...
Для каждой длины L от 1 до 20 перебирали все строки над алфавитом {a, b} длины L (всего 2^L строк).
Для каждой строки измеряли время работы наивного и оптимизированного парсеров (в наносекундах) и проверяли эквивалентность результатов.
Строки разбивали на два пула:
IN — строки, которые принимаются языком,
OUT — строки, которые не принимаются.
Для каждого L просуммировали время обработки строк из IN и OUT отдельно, после чего вычисляли ускорение:
speedup_in(L) = T_slow_in(L) / T_fast_in(L),
speedup_out(L) = T_slow_out(L) / T_fast_out(L).
По полученным значениям построили графики speedup_in(L) и speedup_out(L). 

<img width="600" height="371" alt="T_slow _ T_fast (1)" src="https://github.com/user-attachments/assets/71e52bb5-c1d9-4d32-b52b-90a51c4719c4" />

“График 1 — speedup_out(L) для строк, не принадлежащих языку (OUT)”

<img width="600" height="371" alt="T_slow _ T_fast" src="https://github.com/user-attachments/assets/82266042-a549-4f81-afe5-2f244e5185b4" />

“График 2 — speedup_in(L) для строк, принадлежащих языку (IN)”
Для нечётных L язык пуст (cnt_in = 0), поэтому speedup_in(L) принимается равным 0; ненулевые значения появляются только на чётных L, что даёт такую форму.
