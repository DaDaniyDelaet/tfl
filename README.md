...
Рассмотрим последовательность классов эквивалентности (aс)<sup>k</sup> - бесконечная, т.к. k бесконечная.<br>
Мы получили смежные классы вида *aс*, *aсaс*, *aсaсaс*, ... (aс)<sup>k</sup> и их бесконечно, к ним не применимы левые части правил т.е. НФ, а получить их можно к примеру применением последовательности правил sss-> ac из слов вида (sss)*. <br>
Значит классов эквивалентности бесконечное количество.
...
Дальше слова из *а* и *b* уже рассмоттрены в **5.1**, или если есть *aa* или *bbb* то все слово в *cc*, остается только abb  ↔ ac ↔ cc<br>
Слова длиной больше 2 с *c*: <br>
cab - NF<br>
Причем c не может стоять после *a* или *b* т.к. тогда опять схлопывается в *cc* и *c* не может стоять перед *b* так как тогда случай bbb ↔ cc<br>
=> добавляются классы эквивалентности с *c* и a(ba)<sup>n</sup> или (ab)<sup>n</sup><br>
Это се возможные расположения букв между собой)<br>
Тогда класы экивалентности: <br>

E, c, b, a, cc, ca, (ab)<sup>n</sup>, b(ab)<sup>n</sup>, a(ba)<sup>n</sup>, (ba)<sup>n</sup>, сa(ba)<sup>n</sup>, с(ab)<sup>n</sup> , <br>

- где классы вида (ab)<sup>n</sup> это бесконечный набор классов эквивалентности которые имеют похожую структуру записи (ab, abab, ababab, ... ab*).
Тут получили тоже бесконечное количество классов эквивалентности (несмотря на то что смогли описать все их бесконечные разновидности).

**5.3 Система переписывания**
Тут автомата - фикция, классов эквивалентности бесконечное количество, построенный автомат изначально неправильный, он лишь описывает структуру классов эквивалентности и переходов между ними,
есть состояния описанные переходом к примеру ab, хотя по сути каждая такая пара переходов образует новый класс эквивалентности.
...
Наша система имеет бесконеное колличество классов эквивалентности.
...
aa → bc<br>
bc → abba<br>
сталкиваемся с проблемой в виде bb → s и bb → c т.е. bb уже критическая пара => локальная конфлюэнтность не соблюдается.

Теперь разберем пополняемость по Кнуту–Бендиксу:<br>
найти строгий инвариант (или порядок при котором убывает) не получилось, но тем не менее докажем что система не пополняема по этому алгоритму.<br>
из первого шага выбрав критическую пару bb → s и bb → c, основываясь на порядке, мы бы дополнили систему правилом c → s или s → c, которое перекрывает выбранную критическую пару.<br>
...
bc → abba<br>
c → s<br>
в которой рассматривается следующая очевидная критическая пара cc → ss и cc → cssc,<br>
где в свою очередь к cssc по правилу c → s получим:<br>
cc → cssc → sssc → acc<br>
мы получили cc → acc т.е. дополняя систему подобным правилом мы можем сколько угодно раз добавлять a перед cc (cc → (a)\*cc),<br>
что помимо возникновения бесконечного количества критических пар (не конфлюэнтной) ещё и делает нашу систему бесконечной (не терминируемой) и не завершимой. => подобное правило c → s не подходит.<br>
...
bc → abba<br>
s → c<br>
так же рассмотрим следующую критическую пару sss и конфликт с правилом cc → cssc:<br>
sss → ac и sss → ccs → csscs → ccccs →\* (cc)\*c<br>
аналогично первому пункту система перестаёт терминировать и тем более не конфлюэнтной (количество критических пар снова стремится к бесконечности),<br>
к примеру, пытаясь дальше продолжить алгоритм, придётся вводить правила:<br>
ccc → ac<br>
ccccс → ac<br>
ccccccс → ac<br>
и т.д., снова алгоритм не выполним, т.к. правила cc → cssc и s → c, делают нашу систему не терминированной, вследствие чего алгоритм будет выполняться бесконечно долго.<br>
из 1 и 2 => к нашей системе алгоритм не применим.<br>
(подобный пример был в лекциях про Eq(x, x) → True и Eq(x, x+1) → False, к которой алгоритм не применим).<br>


### если подойти к вопросу пополняемости с логической стороны для пункта 2.
Оставив рассмотрения алгоритма и решить именно вопрос пополняемости, <br>
то как будто можно попробовать компенсировать правилом cccc→ccc, и возможно будет конфлюэнтной по аналогии с псевдо автоматом из прошлого пункта)<br>
(опять же это не относится к алгоритму, т.к. даже в алгоритме критические пары будут отличаться на (cc)*,<br>
т.е. будет вводиться правило ccccс→ccc, компенсирующее сс→cccс, и система будет выглядеть так <br>
bb → s<br>
bb → c<br>
cc → cssc<br>
sss → ac<br>
...
ac → ccc<br>
ccccс → ccc<br>
но и тут возникает куча противоречий к примеру aac→accc→cccсс→ccc = c<sup>3</sup> или  aac→acac→cccссс→cccc = c<sup>4</sup>) <br>
поэтому и это не влияет на возможность пополняемости по Кнуту–Бендиксу, система останется не пополняемой)


## 4. Фаз-тестирование эквивалентности

И всё-таки, чтобы попытаться избежать подобного противоречия и построить терминирующую и конфлюэнтную T`, для нашей системы изменим и добавим правила.<br>
Чтобы попытаться сохранить классы эквивалентности (они были описаны выше вместе с автоматом).<br>
Для начала точно придётся избавиться от критической пары bb → s и bb → c, там в силу ↔ это не создавало проблем, возьмём нашу SRS T и переделаем для well-founded порядка:<br>
берём длино-лексикографический порядок:<br>
a > b > c > s<br>
и перепишем систему в Tn (T-normal удобную для пополнения по Кнуту–Бендиксу):<br>
...
Тогда у нас в силу того, что правый редекс длиннее левого, выполняется терминация, по Tn построим T`, которая будет дополненеа правилами по Кнуту–Бендиксу до конфлюентной, сохраняя конечность применения праил.<br>
Тогда в фаз-тестироании сраним наши Tn и T`.<br>
В файле TestLab1 прикреплен код который сначла апополняет Tn до T` (останавливвается либо при достижении глобальной конфлюентносьти, либо при достиженнии коллличеста праил переписыввания свыше 20), а потом сравнивает их эквиалентность, путем построения случайного слова ω (конечной длины взята до20). Далее строится случаное слово  w' путем применеия правил, и приводится к нормальной форме, и сравниваются нормальные формы внутри каждой из SRS, процедура выполноняется пока не будут найдены контр примеры, или достигнуты ограничения. <br>
Все.
